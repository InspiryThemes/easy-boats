...
class Easy_Boats_Admin {
	 ...
	 */
	public function display_easy_boats_settings() {
		?>
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
				<?php
				settings_fields( 'easy_boats_option_group' );
				do_settings_sections( 'easy_boats_page' );
	 ...
	 */
	public function initialize_easy_boats_options() {

		// If the easy boats options do not exist then create them
		if ( false == $this->easy_boats_options ) {
			add_option( 'easy_boats_options', apply_filters( 'easy_boats_default_options', array( $this, 'easy_boats_default_options' ) ) );
		}
		...
		add_settings_section(
			...
			'easy_boats_page'                                   // Page on which to add this section of options
		);
		...
		add_settings_field(
			...
			array(
				...
				'field_option'      => 'easy_boats_options',
				'field_default'     => '$',
		 ...
		 * Register Settings
		 */
		register_setting( 'easy_boats_option_group', 'easy_boats_options' );
	...
	public function text_option_field( $args ) {
		...
		if ( $field_id ) {
			...
			$field_value = ( isset( $this->easy_boats_options[ $field_id ] ) ) ? $this->easy_boats_options[ $field_id ] : $field_default;
	...
	public function select_option_field( $args ) {
		...
		if ( $field_id ) {
			...
			// Default value or stored value based on option field
			$existing_value = ( isset( $this->easy_boats_options[ $field_id ] ) ) ? $this->easy_boats_options[ $field_id ] : $field_default;

			?>
            <select name="<?php echo $field_option . '[' . $field_id . ']'; ?>"
                    class="inspiry-select-field <?php echo $field_id; ?>">
	 ...
	 */
	function easy_boats_default_options() {
