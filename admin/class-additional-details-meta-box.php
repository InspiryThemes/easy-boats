...
class Additional_Details_Meta_Box {
     ...
     */
	private $nonce = 'easy_boats_additional_details_nonce';
	...
	*/
	...
	public function add_additional_details_meta_box() {
		...
		add_meta_box( 'additional-details-meta-box', __( 'Additional Details', 'easy-boats' ), array( $this, 'display_additional_details' ), 'boat', 'normal', 'core' );
	...
	public function display_additional_details( $post ) {
		...
		/* $detail_titles = get_post_meta($post->ID,'EASYBOATS_detail_titles',true);
		if( !empty( $detail_titles ) ) {
			$detail_values = get_post_meta($post->ID, 'EASYBOATS_detail_values', true);
			if (!empty($detail_values)) {
				$additional_details = array_combine( $detail_titles, $detail_values );
				if ( update_post_meta( $post->ID, 'EASYBOATS_additional_details', $additional_details ) ){
					delete_post_meta( $post->ID, 'EASYBOATS_detail_titles' );
					delete_post_meta( $post->ID, 'EASYBOATS_detail_values' );
				}
			}
		} */

		// nonce field for better security
		wp_nonce_field( 'easy_boats_additional_details_meta_box', $this->nonce );
		...
		<div class="inspiry-details-wrapper">
			...
			<div class="inspiry-detail labels">
				...
				<div class="inspiry-detail-title"><label><?php _e( 'Title', 'easy-boats' ) ?></label></div>
				<div class="inspiry-detail-value"><label><?php _e( 'Value', 'easy-boats' ); ?></label></div>
				<div class="inspiry-detail-control">&nbsp;</div>
			</div>
			...
			<div id="inspiry-additional-details-container">
				...
				$additional_details = get_post_meta( $post->ID, 'EASYBOATS_additional_details', true );
		...
		if( $this->user_can_save( $post_id, $this->nonce ) ) {
			...
			if( isset( $_POST['detail-titles'] ) && isset( $_POST['detail-values'] ) ) {
				...
				if( !empty( $additional_details_titles ) && !empty( $additional_details_values ) ) {
                    ...
                    if( ! empty( $additional_details ) ) {
                        update_post_meta( $post_id, 'EASYBOATS_additional_details', $additional_details );
                        return;
                    }
				}
			}
            ...
            delete_post_meta( $post_id, 'EASYBOATS_additional_details' );
		}
	...
	function user_can_save( $post_id, $nonce ) {
	    ...
	    $is_valid_nonce = ( isset( $_POST[ $nonce ] ) && wp_verify_nonce( $_POST[ $nonce ], 'easy_boats_additional_details_meta_box' ) );
